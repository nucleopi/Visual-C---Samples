...
template<typename T, typename Y>
std::error_code sensor_transmission_base::receive_data(T * rawdata, Y * data, size_t bit_count)
{
...
template<typename T, typename Y>
std::error_code sensor_transmission_base::send_data(T * rawdata, Y * data, size_t bit_count)
{
	if (!data || bit_count > sizeof(T) * 8)
		return std::make_error_code(std::errc::invalid_argument);
